import { byId, clear } from "../core/dom.js";
...
function setActiveLinks(root) {
    ...
    root.querySelectorAll("a[data-url]").forEach((link) => {
        const url = normalizePath(link.getAttribute("data-url"));
        link.classList.toggle("is-active", url === current);
    });
}
...
function buildMenu(doc, statusElement, rootElement) {
    ...
    if (!mainItems.length) {
        statusElement.textContent = "No menu items returned.";
        return;
    }
    ...
    statusElement.textContent = "Menu loaded.";
...
export async function initMenu() {
    ...
    try {
        ...
        statusElement.textContent = "Failed to load menu.";
        clear(rootElement);
        console.error(error);
    }
}