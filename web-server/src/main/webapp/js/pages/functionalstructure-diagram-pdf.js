...
function drawRequirementNode(commands, node, x, y, width, height, scale) {
    ...
    const titleLines = fitPdfTextLines(
        ...
        width - 18 * scale,
        titleFontSize + 2,
        Math.max((titleFontSize + 2) * 1.18 * 2, 14 * scale),
        2
    );
    ...
    const descriptionLines = fitPdfTextLines(
        description || "â€”",
        width - 14 * scale,
        descriptionFontSize,
        Math.max(0, height - headerHeight - 18 * scale),
        5
    );
...
function drawPdfHeader(commands, options) {
    ...
    drawPdfText(
        commands,
        `Functional Architecture ${orientationLabel} Diagram - ${topPanel.projectName || "Project"}`,
        margin,
