...
function buildDiagramPdfFileName(orientation, topPanel) {
    ...
    return `functional-structure-${orientation}-diagram-${projectName}.pdf`;
}
...
function createDiagramPdf({
    ...
    /*
     * Paper format:
     * horizontal diagram = A4 portrait / stÃ¥ende
...
function drawRequirementNode(commands, node, x, y, width, height, scale) {
    const requirement = node.requirement || {};
    const code = requirement.id || node.code || "â€”";
    const name = requirement.name || node.name || "â€”";
    const description = requirement.description || node.description || "";

    const headerHeight = Math.max(18, 34 * scale);
    const codeFontSize = Math.max(3.8, 7.4 * scale);
    const titleFontSize = Math.max(3.8, 7.2 * scale);
    const descriptionFontSize = Math.max(3.2, 6.2 * scale);
    ...
    commands.push("q");
    commands.push(`${formatRgb(hexToRgb("#64748b"))} RG`);
    commands.push(`${formatPdfNumber(Math.max(0.35, 0.7 * scale))} w`);
    commands.push(`${formatPdfNumber(x + 7 * scale)} ${formatPdfNumber(y + height - headerHeight - 4 * scale)} m`);
    commands.push(`${formatPdfNumber(x + width - 7 * scale)} ${formatPdfNumber(y + height - headerHeight - 4 * scale)} l`);
    commands.push("S");
    commands.push("Q");

    const descriptionLines = wrapPdfText(
        description || "â€”",
        Math.max(8, Math.floor((width - 14 * scale) / (descriptionFontSize * 0.52))),
        5
    );
    ...
    drawPdfMultilineText(
        ...
        descriptionLines,
        x + 7 * scale,
        y + height - headerHeight - 14 * scale,
        descriptionFontSize,
        "Helvetica",
        [203, 213, 225],
...
function drawPdfHeader(commands, options) {
    ...
    drawPdfText(
        commands,
        `Functional Structure ${orientationLabel} Diagram - ${topPanel.projectName || "Project"}`,
        margin,
    ...
    drawPdfText(
        commands,
        `${topPanel.customerName || "Customer"} Â· Functional structures: ${requirementCount}`,
        margin,
