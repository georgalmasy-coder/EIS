...
const STORAGE_KEYS = {
    selectedView: "basis.systemrequirement.main.selectedView",
    filterText: "basis.systemrequirement.main.filterText",
    ...
    hiddenColumns: "basis.systemrequirement.main.hiddenColumns",
    groupBy: "basis.systemrequirement.main.groupBy",
    groupCollapsed: "basis.systemrequirement.main.groupCollapsed",
    statusFields: "basis.systemrequirement.main.statusFields"
};
...
async function loadSystemRequirements() {
    ...
    try {
        ...
        const scrollToId = sessionStorage.getItem("basis.systemrequirement.scrollToId");
        if (scrollToId) {
            setTimeout(() => {
...
function openEditRequirement(requirement) {
    ...
    openEditDialog({
        ...
        onSaved: (payload) => {
            ...
            if (targetId) {
                sessionStorage.setItem("basis.systemrequirement.scrollToId", targetId);
            }
            loadSystemRequirements();
        }
    });
...
function openCreateRootRequirement() {
    openEditDialog({
        ...
        onSaved: (payload) => {
            if (payload?.id) {
                sessionStorage.setItem("basis.systemrequirement.scrollToId", payload.id);
            }
            loadSystemRequirements();
        }
    });
...
function scrollToEntity(entityId) {
    ...
    const tryScroll = (attempts = 0) => {
        ...
        if (row) {
            ...
            sessionStorage.removeItem("basis.systemrequirement.scrollToId");
        } else if (attempts < 20) {
