...
const STORAGE_KEYS = {
    ...
    hiddenColumns: "basis.stakeholderrequirement.main.hiddenColumns",
    groupBy: "basis.stakeholderrequirement.main.groupBy",
    groupCollapsed: "basis.stakeholderrequirement.main.groupCollapsed"
};
...
async function loadStakeholderRequirements() {
    ...
    try {
        ...
        const scrollToId = sessionStorage.getItem("basis.stakeholderrequirement.scrollToId");
        if (scrollToId) {
            setTimeout(() => {
...
function renderListView() {
    ...
    tbody.querySelectorAll(".stakeholderrequirement-group-toggle").forEach((button) => {
        button.addEventListener("click", () => {
...
function openEditRequirement(requirement) {
    ...
    openEditDialog({
        ...
        onSaved: (payload) => {
            ...
            if (targetId) {
                sessionStorage.setItem("basis.stakeholderrequirement.scrollToId", targetId);
            }
            loadStakeholderRequirements();
        }
    });
...
function openCreateRootRequirement() {
    openEditDialog({
        ...
        onSaved: (payload) => {
            if (payload?.id) {
                sessionStorage.setItem("basis.stakeholderrequirement.scrollToId", payload.id);
            }
            loadStakeholderRequirements();
        }
    });
...
function scrollToEntity(entityId) {
    ...
    const tryScroll = (attempts = 0) => {
        ...
        if (row) {
            ...
            sessionStorage.removeItem("basis.stakeholderrequirement.scrollToId");
        } else if (attempts < 20) {
