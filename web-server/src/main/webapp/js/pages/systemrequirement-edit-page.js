...
const SUMMARY_FIELDS = {
    code: ["SystemReqCode", "SystemRequirementCode", "RequirementCode", "Code", "EntityId"],
    name: ["RequirementName", "SystemRequirementName", "Name"]
};
...
async function loadDetail() {
    ...
    setText("loadStatus", "Loading");
    setText("dlgStatus", "Loading system requirement details…");
    ...
    try {
        ...
        console.error("Failed to load system requirement detail", error);
        setText("loadStatus", "Error");
        setText("dlgStatus", `Could not load system requirement detail. ${error.message}`);
    }
}
...
function buildDetailUrl() {
    if (state.mode === MODES.edit) {
        if (!state.id) return "";
        return `/basis/systemrequirement?cmd=edit&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.editVersion) {
        if (!state.id || !state.version) return "";
        return `/basis/systemrequirement?cmd=edit&id=${encodeURIComponent(state.id)}&version=${encodeURIComponent(state.version)}`;
    }

    if (state.mode === MODES.createChild) {
        if (!state.id) return "";
        return `/basis/systemrequirement?cmd=create&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.createRoot) {
        return "/basis/systemrequirement?cmd=create";
    }
...
async function saveCurrentRequirement() {
    ...
    try {
        ...
        console.error("Failed to save system requirement", error);
