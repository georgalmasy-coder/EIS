...
const STORAGE_KEYS = {
    selectedView: "basis.requirements.stakeholder.selectedView",
    filterText: "basis.requirements.stakeholder.filterText",
    activeOnly: "basis.requirements.stakeholder.activeOnly",
    sortKey: "basis.requirements.stakeholder.sortKey",
    sortDirection: "basis.requirements.stakeholder.sortDirection",
...
const VIEW_PAGE_URLS = {
    list: "/web/view?page=stakeholderrequirement-main",
...
export class StakeholderRequirementController {
    ...
    openImport() {
        const dlg = document.getElementById("stakeholderImportDialog");
        if (dlg && dlg.showModal) dlg.showModal();
    }
    openExport() {
        const dlg = document.getElementById("stakeholderExportDialog");
        if (dlg && dlg.showModal) dlg.showModal();
    }
    addRoot() {
...
function initializeStateFromStorage() {
    ...
    const filterInput = document.getElementById("stakeholderSearchInput");
    const activeOnlyInput = document.getElementById("stakeholderFilterActiveOnly");
...
function initializeEvents() {
    ...
    const activeOnlyInput = document.getElementById("stakeholderFilterActiveOnly");
    const clearFilterButton = document.getElementById("stakeholderBtnClearFilter");
    const stakeholderGroupByZone = document.getElementById("stakeholderGroupByZone");
    ...
    document.getElementById("stakeholderBtnColumns")?.addEventListener("click", () => {
        toggleColumnsMenu();
    });

    document.getElementById("stakeholderBtnAddRoot")?.addEventListener("click", () => {
        openCreateRootRequirement();
    });
    ...
    stakeholderGroupByZone?.addEventListener("dragenter", () => {
        stakeholderGroupByZone.classList.add("is-drag-over");
    });

    stakeholderGroupByZone?.addEventListener("dragover", (event) => {
        event.preventDefault();
        event.dataTransfer.dropEffect = "copy";
    });

    stakeholderGroupByZone?.addEventListener("dragleave", (event) => {
        if (event.target === stakeholderGroupByZone) {
            stakeholderGroupByZone.classList.remove("is-drag-over");
        }
    });

    stakeholderGroupByZone?.addEventListener("drop", (event) => {
        event.preventDefault();
        stakeholderGroupByZone.classList.remove("is-drag-over");
        addGroupByKey(event.dataTransfer.getData("text/plain"));
    });

    stakeholderGroupByZone?.addEventListener("click", (event) => {
        const target = event.target;
        const clearBtn = target.closest("#stakeholderBtnClearGrouping");
        if (clearBtn) {
            event.preventDefault();
        ...
        const removeBtn = target.closest(".stakeholderrequirement-group-chip button");
        if (removeBtn) {
            event.preventDefault();
            event.stopPropagation();
            const chip = removeBtn.closest(".stakeholderrequirement-group-chip");
    ...
    stakeholderColumnsList?.addEventListener("change", (event) => {
...
function initializeImportExportDialogs() {
    const exportDialog = createExportDialog({
        dialogId: "stakeholderExportDialog",
        openButtonId: "stakeholderBtnExport",
    ...
    const importDialog = createImportDialog({
        dialogId: "stakeholderImportDialog",
        openButtonId: "stakeholderBtnImport",
...
function applyFiltersAndRender() {
    const filterText = (document.getElementById("stakeholderSearchInput")?.value || "").trim().toLowerCase();
    const activeOnly = document.getElementById("stakeholderFilterActiveOnly")?.checked !== false;
...
function applyView(viewType, options = {}) {
    ...
    const section = document.getElementById("stakeholderSection");
...
function updateActionButtonsForView(viewType) {
    ...
    setElementHidden("stakeholderBtnImport", !isListView || hasRequirements);
...
function persistFilters() {
    const filterText = document.getElementById("stakeholderSearchInput")?.value || "";
    const activeOnly = document.getElementById("stakeholderFilterActiveOnly")?.checked !== false;
...
function renderstakeholderGroupByZone() {
    const groupZone = document.getElementById("stakeholderGroupByZone");
    ...
    const clearButtonMarkup = `
        <button
                id="stakeholderBtnClearGrouping"
                class="stakeholderrequirement-grouping-clear"
                type="button"
    ...
    if (!state.groupBy.length) {
        ...
        groupZone.innerHTML = `${state.groupBy.map((key) => `
            <span class="stakeholderrequirement-group-chip" draggable="true" data-group-key="${escapeHtml(key)}">
    ...
    groupZone.querySelectorAll(".stakeholderrequirement-group-chip").forEach((chip) => {
...
function renderColumnsMenu() {
    const stakeholderColumnsList = document.getElementById("stakeholderColumnsList");
    const columnsButton = document.getElementById("stakeholderBtnColumns");

    if (!stakeholderColumnsList || !columnsButton) {
        return;
    }

    const visibleCount = getVisibleListColumnCount();

    stakeholderColumnsList.innerHTML = state.listColumns.map((column) => {
        const checked = !state.hiddenColumns.includes(column.key);
        const disableToggle = checked && visibleCount <= 1;

        return `
            <label class="stakeholderrequirement-column-option ${checked ? "" : "is-hidden"}">
                <input
                        type="checkbox"
                ...
                >
                <span class="stakeholderrequirement-column-option-label">${escapeHtml(column.label)}</span>
            </label>
        `;
    }).join("");
...
function openColumnsMenu() {
    const stakeholderColumnsPopover = document.getElementById("stakeholderColumnsPopover");
    const columnsButton = document.getElementById("stakeholderBtnColumns");

    if (!stakeholderColumnsPopover || !columnsButton) {
    ...
    renderColumnsMenu();
    stakeholderColumnsPopover.hidden = false;
...
function closeColumnsMenu() {
    ...
    if (stakeholderColumnsPopover) {
        stakeholderColumnsPopover.hidden = true;
    }
...
function toggleColumnsMenu() {
    ...
    if (!stakeholderColumnsPopover || stakeholderColumnsPopover.hidden) {
        openColumnsMenu();
        return;
    }
...
function handleDocumentPointerDown(event) {
    ...
    if (stakeholderColumnsControl.contains(event.target)) {
        return;
    }
...
function renderListView() {
    const colGroup = document.getElementById("stakeholderColGroup");
    const headerRow = document.getElementById("stakeholderHeaderRow");
    const stakeholdertbody = document.getElementById("stakeholderTbody");
    const table = document.querySelector(".stakeholderrequirement-table");

    if (!colGroup || !headerRow || !stakeholdertbody) {
        return;
    }
    ...
    headerRow.innerHTML = columns.map((column) => {
        ...
        return `
            <th data-key="${escapeHtml(column.key)}" draggable="true" class="stakeholderrequirement-resizable-th">
    ...
    stakeholdertbody.innerHTML = state.groupBy.length
    ...
    stakeholdertbody.querySelectorAll("tr[data-requirement-index]").forEach((row) => {
    ...
    stakeholdertbody.querySelectorAll(".stakeholderrequirement-group-toggle").forEach((button) => {
        button.addEventListener("click", () => {
...
function renderGroupedRows(rows, columns, depth = 0, pathParts = []) {
    ...
    return Array.from(groups.entries())
        ...
        .map(([groupValue, groupRows]) => {
            ...
            return `
                <tr class="stakeholderrequirement-group-row ${collapsed ? "is-collapsed" : ""}" data-group-path="${escapeHtml(groupPath)}">
                    <td colspan="${columns.length}">
                        <div class="stakeholderrequirement-group-row-inner">
                            <button
                                    type="button"
                                    class="stakeholderrequirement-group-toggle"
                                    data-group-path="${escapeHtml(groupPath)}"
                                    aria-label="${collapsed ? "Expand group" : "Collapse group"}"
                                    aria-expanded="${collapsed ? "false" : "true"}"
                            >${collapsed ? "+" : "-"}</button>
                            <span class="stakeholderrequirement-group-label">${escapeHtml(labelForGroupKey(groupKey))}: ${escapeHtml(groupValue)}</span>
                            <span class="stakeholderrequirement-group-count">(${groupRows.length})</span>
                        </div>
                    </td>
                </tr>
                ${nextRows}
...
function renderListCell(requirement, column) {
    ...
    const isCodeColumn = ["StakeholderReqCode", "StakeholderRequirementCode", "RequirementCode", "Code"].includes(column.key);
...
function renderDiagramView(orientation) {
    const nodesContainer = document.getElementById(`stakeholder${orientation.charAt(0).toUpperCase() + orientation.slice(1)}DiagramNodes`);
    const svg = document.getElementById(`stakeholder${orientation.charAt(0).toUpperCase() + orientation.slice(1)}DiagramSvg`);
    const canvas = document.getElementById(`stakeholder${orientation.charAt(0).toUpperCase() + orientation.slice(1)}DiagramCanvas`);
...
function openContextMenu(x, y, targetType, requirement) {
    const menu = document.getElementById("stakeholderRequirementContextMenu");
    const editButton = document.getElementById("stakeholderContextMenuEditRequirement");
    const createSubButton = document.getElementById("stakeholderContextMenuCreateSubRequirement");
    const createRootButton = document.getElementById("stakeholderContextMenuCreateRootRequirement");
...
function openEditRequirement(requirement) {
    ...
    openEditDialog({
        ...
        onSaved: (payload) => {
            ...
            if (targetId) {
                sessionStorage.setItem("basis.requirements.stakeholder.scrollToId", targetId);
            }
            loadStakeholderRequirements();
        }
    });
...
function openCreateRootRequirement() {
    openEditDialog({
        ...
        onSaved: (payload) => {
            if (payload?.id) {
                sessionStorage.setItem("basis.requirements.stakeholder.scrollToId", payload.id);
            }
            loadStakeholderRequirements();
        }
    });
...
function scrollToEntity(entityId) {
    ...
    const tryScroll = (attempts = 0) => {
        ...
        if (row) {
            ...
            sessionStorage.removeItem("basis.requirements.stakeholder.scrollToId");
        } else if (attempts < 20) {
...
function showListEmptyState(message) {
    const emptyState = document.getElementById("stakeholderListEmptyState");
...
function showDiagramEmptyState(orientation, message) {
    const emptyState = document.getElementById(`stakeholder${orientation.charAt(0).toUpperCase() + orientation.slice(1)}DiagramEmptyState`);
