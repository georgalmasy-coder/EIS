...
const SUMMARY_FIELDS = {
    code: ["SBSCode", "SystemCode", "SystemBreakdownCode", "Code", "EntityId"],
    name: ["SystemName", "SystemBreakdownName", "Name"]
};
...
function buildDetailUrl() {
    if (state.mode === MODES.edit) {
        if (!state.id) return "";
        return `/project/systembreakdown?cmd=edit&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.editVersion) {
        if (!state.id || !state.version) return "";
        return `/project/systembreakdown?cmd=edit&id=${encodeURIComponent(state.id)}&version=${encodeURIComponent(state.version)}`;
    }

    if (state.mode === MODES.createChild) {
        if (!state.id) return "";
        return `/project/systembreakdown?cmd=create&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.createRoot) {
        return "/project/systembreakdown?cmd=create";
    }
...
function applyModeUi() {
    ...
    document.body.classList.toggle("systemsbreakdown-edit-readonly", state.readOnly);
...
function getModeLabel() {
    ...
    if (state.mode === MODES.createChild) return "Create Sub System";
    if (state.mode === MODES.createRoot) return "Create Root System";
    return "Edit System";
}
...
function renderBasisInfoFieldMarkup(field) {
    ...
    if (control === "textarea" || name === "SystemDescription" || name === "SystemBreakdownDescription") {
        return `
...
async function saveCurrentSystem() {
    ...
    const validationErrors = validateCurrentSystem();
...
function buildSavePayload() {
    ...
    const detailNode = detailContainer
        ? getDirectChild(detailContainer, "systembreakdown")
        : getDirectChild(root, "systembreakdown");
...
function initializeResizableEditTable(table, config) {
    ...
    headerCells.forEach((headerCell, columnIndex) => {
        headerCell.classList.add("systemsbreakdown-edit-resizable-th");

        if (headerCell.querySelector(":scope > .systemsbreakdown-edit-column-resizer")) {
            return;
        }

        const handle = document.createElement("span");
        handle.className = "systemsbreakdown-edit-column-resizer";
...
function startEditTableColumnResize(event, table, config, columnIndex, headerCell) {
    ...
    document.body.classList.add("systemsbreakdown-edit-column-resizing");
    ...
    function onMouseUp(upEvent) {
        ...
        document.body.classList.remove("systemsbreakdown-edit-column-resizing");
