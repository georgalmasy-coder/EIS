...
const SAVE_URL = "/pro/logicalstructure?cmd=save";
const EDIT_PAGE_URL = "/web/view?page=logicalstructure-edit";
const DEFAULT_RETURN_URL = "/web/view?page=logicalstructure-main";
const RELATION_LIST_URL = "/pro/logicalstructure/relationlist";
const RELATION_ENTITY_TYPE_ID = 8;

const STORAGE_KEYS = {
    tableColumnWidths: "pro.logicalstructure.edit.tableColumnWidths"
};
...
const SUMMARY_FIELDS = {
    code: ["LogicalCode", "Code", "EntityId"],
...
async function loadDetail() {
    ...
    setText("loadStatus", "Loading");
    setText("dlgStatus", "Loading logical structure details…");
    ...
    try {
        ...
        console.error("Failed to load logical structure detail", error);
        setText("loadStatus", "Error");
        setText("dlgStatus", `Could not load logical structure detail. ${error.message}`);
    }
}
...
function buildDetailUrl() {
    if (state.mode === MODES.edit) {
        if (!state.id) return "";
        return `/pro/logicalstructure?cmd=edit&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.editVersion) {
        if (!state.id || !state.version) return "";
        return `/pro/logicalstructure?cmd=edit&id=${encodeURIComponent(state.id)}&version=${encodeURIComponent(state.version)}`;
    }

    if (state.mode === MODES.createChild) {
        if (!state.id) return "";
        return `/pro/logicalstructure?cmd=create&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.createRoot) {
        return "/pro/logicalstructure?cmd=create";
    }
...
function applyModeUi() {
    ...
    document.body.classList.toggle("logicalstructure-edit-readonly", state.readOnly);
...
function findDetailNode(root) {
    return root?.querySelector("logicalStructureDocument > logicalDocument")
        || root?.querySelector("logicalStructureDocument logicalDocument")
        || root?.querySelector("logicalStructureDocument > logicalStructure")
        || root?.querySelector("logicalStructureDocument logicalStructure")
        || root?.querySelector("logicalDocument")
        || root?.querySelector("logicalStructure")
        || null;
}
...
async function saveCurrentRequirement() {
    ...
    try {
        ...
        console.error("Failed to save logical structure", error);
...
function buildSavePayload() {
    ...
    const detailContainer = getDirectChild(root, "logicalStructureDocument")
        || getDirectChild(root, "logicalStructure");
    const detailNode = detailContainer
        ? getDirectChild(detailContainer, "logicalDocument") || getDirectChild(detailContainer, "logicalStructure")
        : getDirectChild(root, "logicalDocument") || getDirectChild(root, "logicalStructure");
...
function initializeResizableEditTable(table, config) {
    ...
    headerCells.forEach((headerCell, columnIndex) => {
        headerCell.classList.add("logicalstructure-edit-resizable-th");

        if (headerCell.querySelector(":scope > .logicalstructure-edit-column-resizer")) {
            return;
        }

        const handle = document.createElement("span");
        handle.className = "logicalstructure-edit-column-resizer";
...
function startEditTableColumnResize(event, table, config, columnIndex, headerCell) {
    ...
    document.body.classList.add("logicalstructure-edit-column-resizing");
    ...
    function onMouseUp(upEvent) {
        ...
        document.body.classList.remove("logicalstructure-edit-column-resizing");
