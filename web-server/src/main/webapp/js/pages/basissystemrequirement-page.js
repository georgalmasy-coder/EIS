...
function findDetailNode(root) {
    return root.querySelector("systemRequirementDocument > systemRequirement")
        || root.querySelector("systemRequirementDocument systemRequirement")
        || root.querySelector("systemRequirement");
}
...
function renderBasisInfoFieldMarkup(field) {
    ...
    if (control === "textarea" || name === "RequirementDescription" || name === "SystemRequirementDescription") {
        return `
...
function renderBasisInfoFromDoc(doc) {
    const root = doc?.documentElement || doc;
    const detailContainer = root.getElementsByTagName("systemRequirementDocument")?.[0] || null;
    const detailNode = detailContainer
        ? detailContainer.getElementsByTagName("systemRequirement")?.[0] || null
        : root.getElementsByTagName("systemRequirement")?.[0] || null;
...
function buildBasisSystemRequirementSavePayload(elements, context) {
    ...
    const detailContainer = getDirectChild(root, "systemRequirementDocument");
    const detailNode = detailContainer
        ? getDirectChild(detailContainer, "systemRequirement")
        : getDirectChild(root, "systemRequirement");
...
function start() {
    ...
    const exportDialog = createExportDialog({
        ...
        exportUrl: "/basis/systemrequirement",
        baseFileName: "systemrequirements"
    });
    ...
    page = createGenericPage({
        listUrl: "/basis/systemrequirement?cmd=list",
        detailUrl: "/basis/systemrequirement?cmd=edit&id=",
        saveUrl: "/basis/systemrequirement?cmd=save",
        rootListTag: "BasisSystemRequirementList",
        listEntityTag: "systemRequirements",
        detailEntityTag: "systemRequirement",
        detailContainerTag: "systemRequirementDocument",
        findDetailNode,
        ...
        buildSavePayload: buildBasisSystemRequirementSavePayload
    });
