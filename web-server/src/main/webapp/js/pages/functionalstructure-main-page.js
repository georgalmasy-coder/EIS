...
const LIST_URL = "/pro/functionalstructure?cmd=list";
const EDIT_PAGE_URL = "/web/view?page=functionalstructure-edit";

const STORAGE_KEYS = {
    selectedView: "pro.functionalstructure.main.selectedView",
    filterText: "pro.functionalstructure.main.filterText",
    activeOnly: "pro.functionalstructure.main.activeOnly",
    sortKey: "pro.functionalstructure.main.sortKey",
    sortDirection: "pro.functionalstructure.main.sortDirection",
...
const VIEW_PAGE_URLS = {
    list: "/web/view?page=functionalstructure-main",
    horizontal: "/web/view?page=functionalstructure-main-horizontal",
    vertical: "/web/view?page=functionalstructure-main-vertical"
};
...
function initializeEvents() {
    ...
    moveSelection = createEntityMoveSelection({ menuId: "functionalStructureContextMenu", entityType: "fsys" });
...
function initializeImportExportDialogs() {
    ...
    const importDialog = createImportDialog({
        ...
        importUrl: "/pro/functionalstructure?cmd=import",
        onImportComplete: async () => {
            await loadFunctionalStructures();
        }
    });
...
async function loadFunctionalStructures() {
    ...
    try {
        ...
        const scrollToId = sessionStorage.getItem("basis.functionalstructure.scrollToId");
        if (scrollToId) {
            setTimeout(() => {
...
function parseFunctionalStructures(xmlDocument) {
    const requirementNodes = Array.from(
        xmlDocument.querySelectorAll("functionStructures > functionalDocument, functionStructures > functionStructure, functionalDocument, functionStructure")
    );
    ...
    return requirementNodes.map((node, index) => {
        ...
        const entityId = getFirstFieldRawValue(node, [
            "EntityId",
            "FunctionalStructureId",
            "FunctionalStructurePK"
        ], "");
...
function renderColumnsMenu() {
    ...
    columnsList.innerHTML = state.listColumns.map((column) => {
        ...
        return `
            <label class="functionalstructure-column-option ${checked ? "" : "is-hidden"}">
                <input
                        type="checkbox"
                ...
                >
                <span class="functionalstructure-column-option-label">${escapeHtml(column.label)}</span>
            </label>
        `;
    }).join("");
...
function renderGroupByZone() {
    ...
    const clearButtonMarkup = `
        <button
                id="btnClearGrouping"
                class="functionalstructure-grouping-clear"
                type="button"
    ...
    if (!state.groupBy.length) {
        ...
        groupZone.innerHTML = `${state.groupBy.map((key) => `
            <span class="functionalstructure-group-chip" draggable="true" data-group-key="${escapeHtml(key)}">
    ...
    groupZone.querySelectorAll(".functionalstructure-group-chip").forEach((chip) => {
...
function renderListView() {
    ...
    const table = document.querySelector(".functionalstructure-table");
    ...
    headerRow.innerHTML = columns.map((column) => {
        ...
        return `
            <th data-key="${escapeHtml(column.key)}" draggable="true" class="functionalstructure-resizable-th">
                <span class="sort">${escapeHtml(column.label)} <span class="sort-indicator">${indicator}</span></span>
                <span class="functionalstructure-column-resizer" data-resize-column="${escapeHtml(column.key)}" aria-hidden="true"></span>
            </th>
        `;
    }).join("");
    ...
    headerRow.querySelectorAll("th[data-key]").forEach((header) => {
        header.addEventListener("click", (event) => {
            if (event.target.closest(".functionalstructure-column-resizer")) {
                return;
            }
    ...
    tbody.querySelectorAll(".functionalstructure-group-toggle").forEach((button) => {
        button.addEventListener("click", () => {
    ...
    if (rows.length === 0) {
        showListEmptyState("No functional structures match the current filters.");
    } else {
        hideListEmptyState();
    }
...
function renderGroupedRows(rows, columns, depth = 0, pathParts = []) {
    ...
    return Array.from(groups.entries())
        ...
        .map(([groupValue, groupRows]) => {
            ...
            return `
                <tr class="functionalstructure-group-row ${collapsed ? "is-collapsed" : ""}" data-group-path="${escapeHtml(groupPath)}">
                    <td colspan="${columns.length}">
                        <div class="functionalstructure-group-row-inner">
                            <button
                                    type="button"
                                    class="functionalstructure-group-toggle"
                                    data-group-path="${escapeHtml(groupPath)}"
                                    aria-label="${collapsed ? "Expand group" : "Collapse group"}"
                                    aria-expanded="${collapsed ? "false" : "true"}"
                            >${collapsed ? "+" : "-"}</button>
                            <span class="functionalstructure-group-label">${escapeHtml(labelForGroupKey(groupKey))}: ${escapeHtml(groupValue)}</span>
                            <span class="functionalstructure-group-count">(${groupRows.length})</span>
                        </div>
                    </td>
                </tr>
                ${nextRows}
...
function initializeColumnResize(headerRow, colGroup, table, columns) {
    const handles = Array.from(headerRow.querySelectorAll(".functionalstructure-column-resizer"));
    ...
    handles.forEach((handle) => {
        handle.addEventListener("mousedown", (event) => {
            ...
            document.body.classList.add("functionalstructure-column-resizing");
            ...
            function onMouseUp(upEvent) {
                ...
                document.body.classList.remove("functionalstructure-column-resizing");
...
function renderListCell(requirement, column) {
    ...
    if (column.isActiveColumn) {
        return `
            <td title="${requirement.active ? "Active" : "Inactive"}">
                <span class="functionalstructure-active-state" aria-label="${requirement.active ? "Active" : "Inactive"}">
                    <span class="functionalstructure-active-dot ${requirement.active ? "is-active" : "is-inactive"}" aria-hidden="true"></span>
                </span>
            </td>
        `;
    }
    ...
    const isCodeColumn = ["FunctionalCode"].includes(column.key);
...
function createSvgPath(edge, orientation) {
    ...
    path.setAttribute("class", "functionalstructure-diagram-edge");
...
function createDiagramNode(node) {
    ...
    element.className = `functionalstructure-diagram-node${node.type === "project" ? " is-project-node" : ""}`;
...
function initializeContextMenuEvents() {
    const menu = document.getElementById("functionalStructureContextMenu");
...
function openEditRequirement(requirement) {
    ...
    openEditDialog({
        ...
        onSaved: (payload) => {
            ...
            if (targetId) {
                sessionStorage.setItem("basis.functionalstructure.scrollToId", targetId);
            }
            loadFunctionalStructures();
        }
    });
...
function buildEditPageUrl(params) {
    ...
    url.searchParams.set("returnUrl", "/web/view?page=functionalstructure-main");
...
function downloadCurrentDiagramPdf() {
    ...
    downloadFunctionalStructureDiagramPdf({
        tree,
        layout,
        orientation,
...
function scrollToEntity(entityId) {
    ...
    const tryScroll = (attempts = 0) => {
        ...
        if (row) {
            ...
            sessionStorage.removeItem("basis.functionalstructure.scrollToId");
        } else if (attempts < 20) {
