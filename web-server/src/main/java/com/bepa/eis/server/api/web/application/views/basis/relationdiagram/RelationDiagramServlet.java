...
@WebServlet(name = "RelationDiagramServlet", urlPatterns = {"/basis/relationdiagram"})
@MultipartConfig
public class RelationDiagramServlet extends GenericDataProviderServlet {

    private static final Logger log = LoggerFactory.getLogger(RelationDiagramServlet.class);
    ...
    private GenericXmlDocument buildRelationDiagram(WebSession webSession) {
        try {
            return new RelationDiagramDocument(webSession);
        } catch (Exception e) {
            log.error("Error getting relation diagram for stakeholder/system requirements: {}", e.getMessage(), e);
