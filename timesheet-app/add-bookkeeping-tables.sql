...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'ux_accounts_system_key' AND object_id = OBJECT_ID(N'dbo.accounts'))
BEGIN
...
IF OBJECT_ID('dbo.accounting_lines', 'U') IS NULL
BEGIN
...
IF OBJECT_ID('dbo.invoice_approvals', 'U') IS NULL
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'ix_accounting_entries_period' AND object_id = OBJECT_ID(N'dbo.accounting_entries'))
    CREATE INDEX ix_accounting_entries_period ON dbo.accounting_entries(accounting_year, accounting_month, entry_date);
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'ix_accounting_lines_entry' AND object_id = OBJECT_ID(N'dbo.accounting_lines'))
...
) AS source (account_number, account_name, account_type, system_key)
ON target.system_key = source.system_key
WHEN NOT MATCHED THEN
    INSERT (account_number, account_name, account_type, system_key)
    VALUES (source.account_number, source.account_name, source.account_type, source.system_key);
GO
