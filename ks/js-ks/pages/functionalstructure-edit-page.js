...
const SAVE_URL = "/pro/functionalstructure?cmd=save";
const EDIT_PAGE_URL = "/web/view?page=functionalstructure-edit";
const DEFAULT_RETURN_URL = "/web/view?page=functionalstructure-main";
const RELATION_LIST_URL = "/pro/functionalstructure/relationlist";
const RELATION_ENTITY_TYPE_ID = 9;

const STORAGE_KEYS = {
    tableColumnWidths: "pro.functionalstructure.edit.tableColumnWidths"
};
...
async function loadDetail() {
    ...
    try {
        ...
        setText("loadStatus", "Error");
        setText("dlgStatus", `Could not load functional structure detail. ${error.message}`);
    }
}
...
function buildDetailUrl() {
    if (state.mode === MODES.edit) {
        if (!state.id) return "";
        return `/pro/functionalstructure?cmd=edit&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.editVersion) {
        if (!state.id || !state.version) return "";
        return `/pro/functionalstructure?cmd=edit&id=${encodeURIComponent(state.id)}&version=${encodeURIComponent(state.version)}`;
    }

    if (state.mode === MODES.createChild) {
        if (!state.id) return "";
        return `/pro/functionalstructure?cmd=create&id=${encodeURIComponent(state.id)}`;
    }

    if (state.mode === MODES.createRoot) {
        return "/pro/functionalstructure?cmd=create";
    }
...
function applyModeUi() {
    ...
    document.body.classList.toggle("functionalstructure-edit-readonly", state.readOnly);
...
function findDetailNode(root) {
    return root?.querySelector("functionStructureDocument > functionalDocument")
        || root?.querySelector("functionStructureDocument functionalDocument")
        || root?.querySelector("functionStructureDocument > functionStructure")
        || root?.querySelector("functionStructureDocument functionStructure")
        || root?.querySelector("functionalDocument")
        || root?.querySelector("functionStructure")
        || null;
}
...
async function saveCurrentRequirement() {
    ...
    try {
        ...
        console.error("Failed to save functional structure", error);
...
function buildSavePayload() {
    ...
    const detailContainer = getDirectChild(root, "functionStructureDocument")
        || getDirectChild(root, "functionStructure");
    const detailNode = detailContainer
        ? getDirectChild(detailContainer, "functionalDocument") || getDirectChild(detailContainer, "functionStructure")
        : getDirectChild(root, "functionalDocument") || getDirectChild(root, "functionStructure");
...
function initializeResizableEditTable(table, config) {
    ...
    headerCells.forEach((headerCell, columnIndex) => {
        headerCell.classList.add("functionalstructure-edit-resizable-th");

        if (headerCell.querySelector(":scope > .functionalstructure-edit-column-resizer")) {
            return;
        }

        const handle = document.createElement("span");
        handle.className = "functionalstructure-edit-column-resizer";
...
function startEditTableColumnResize(event, table, config, columnIndex, headerCell) {
    ...
    document.body.classList.add("functionalstructure-edit-column-resizing");
    ...
    function onMouseUp(upEvent) {
        ...
        document.body.classList.remove("functionalstructure-edit-column-resizing");
