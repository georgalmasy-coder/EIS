...
function buildMenu(doc, statusElement, rootElement) {
    ...
    mainItems.forEach((mainItem) => {
        ...
        if (mainType === 1) {
            ...
            spacer.className = "menu-header-spacer";
            ...
            if (subItems.length) {
                ...
                subItems.forEach((subItem) => {
                    ulSub.appendChild(createSubMenuItem(subItem));
                });
...
function createSidebarBrand(doc) {
    ...
    brand.innerHTML = `
        <img class="menu-brand-logo" src="/svg/eis-logo-dark.svg" alt="EIS Engineering In Systems">
    `;
...
function initializeHybridMenu() {
    ...
    const brand = createSidebarBrand(document);
    sideMenu.insertBefore(brand, sideMenu.firstChild);

    pinButton = createPinButton(document);
    sideMenu.insertBefore(pinButton, brand.nextSibling);
